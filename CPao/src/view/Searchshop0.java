...
public class Searchshop0 extends JFrame {
	...
	public Searchshop0() {
		...
		lbljie.setFont(new Font("ÀŒÃÂ", Font.BOLD, 20));
		
		JTable table = null;
		DefaultTableModel model =new DefaultTableModel() ;
		model.setRowCount( 0 );
		model.setColumnIdentifiers(new Object[]{"…ÃµÍ", "‰Ø¿¿µÍ∆Ã"});
  		ModelFactory m=new ModelFactory();
  		m.searchshop(MainJMunregistered.name);
  		String[] shopn;
  		String one = null;
  		shopn=m.shopname;
		...
		table.getColumnModel().getColumn(1).setCellEditor(new MyRender4(table));
  		table.getColumnModel().getColumn(1).setCellRenderer(new MyRender4(table));
